class User < ActiveRecord::Base
  ...
  def self.top_rated
    self.select('users.*'). # Select all atrributes of the user
    ...
    select('COUNT(DISTINCT comments.id) + COUNT(DISTINCT posts.id) AS rank'). # Adding comments ans posts together which equals rank
    joins(:posts). # Ties the posts tabe to the users table, via the user_id
    ...
    order('rank DESC') # Instructs the database to order the results in descending order, by the rank we created
  end
